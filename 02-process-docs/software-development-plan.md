...
软件工程环境按前端、后端和文档三个独立仓库组织，根目录通过 `just` 提供统一状态检查、启动、验证和 E2E 入口。前端采用 Next.js + React + TypeScript，后端采用 Spring Boot + Java，文档站采用 VitePress。

#### 6.2.2 软件测试环境

测试环境分为本地开发环境、集成测试环境和浏览器 E2E 环境。集成测试需要 PostgreSQL、RabbitMQ、MinIO、Redis 和 go-judge 等依赖；浏览器 E2E 面向运行中的真实前后端执行。

#### 6.2.3 软件开发库

源代码、接口文档、过程文档和验证脚本分别在对应仓库中维护。构建产物、镜像和部署配置按发布流程归档，不把本地临时文件纳入交付库。

#### 6.2.4 软件开发文档

需求、概要设计、详细设计、测试报告、部署文档、用户手册和大模型使用说明按阶段维护，并在需求、接口、部署或测试口径变化时同步更新。
