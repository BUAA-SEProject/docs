...
本说明书用于定义一体化在线教学与实训平台 V1 的业务目标、范围边界、功能需求、外部接口需求、数据需求、非功能需求和验收依据。本文档回答的是“系统必须做什么、做到什么程度、如何验证”，而不是“系统内部如何设计或实现”。
...
本平台是面向高校、职业培训机构和企业培训部门的一体化在线教学与实训平台。平台以浏览器为主要入口，通过统一的课程、任务、在线提交、自动评测、人工批改、成绩发布和审计治理能力，承载正式教学与训练活动，而不是一次性演示工具。
...
| --- | --- | --- |
| 浏览器客户端 | 用户访问页面、提交表单、查看结果 | 需支持受保护资源访问控制、会话管理和错误反馈 |
...
| FR-CFG-03 | 组织结构维护 | 平台管理员应能维护院系、班级、部门或团队等组织层级，并将用户归属到指定组织节点 | MUST | D/T |
| FR-CFG-04 | 用户导入与创建 | 平台管理员应能批量导入、手动创建和邀请用户 | MUST | T/I |
| FR-CFG-05 | 角色与账号状态管理 | 平台管理员应能分配平台角色，并管理账号启用、停用、锁定与解锁 | MUST | T/I |
| FR-CFG-06 | 配置版本化 | 平台配置应支持草稿保存、发布生效、历史回溯和发布留痕 | MUST | T/I |
| FR-CFG-07 | 平台级资源策略 | 平台应支持配置评测并发、存储额度和通知开关等资源策略 | SHOULD | D/T |

**业务规则：**

- BR-CFG-01：同一时刻只能有一个平台配置版本处于生效状态。
- BR-CFG-02：组织结构默认最多支持 5 级层级嵌套。
- BR-CFG-03：教师、助教和学员必须归属于至少一个组织单元。
- BR-CFG-04：用户批量导入必须返回逐行校验结果和失败原因。
- BR-CFG-05：配置回退仅影响配置展示与功能开关，不得删除既有业务数据。

### 4.2 身份认证与授权

| 编号 | 需求 | 描述 | 优先级 | 验证 |
| --- | --- | --- | --- | --- |
| FR-IAM-01 | 登录与退出 | 平台应支持用户安全登录、保持会话状态并安全退出 | MUST | T |
| FR-IAM-02 | 基于角色的授权 | 系统应按平台角色和课程角色控制页面、操作和数据访问范围 | MUST | T/I |
| FR-IAM-03 | 未授权访问拦截 | 未登录或无权限用户不得访问受保护资源；系统应给出重定向或拒绝结果 | MUST | T |
...
- BR-IAM-03：用户会话超时时间默认 2 小时，可由管理员配置。
...
| FR-SUB-02 | 提交形式支持 | 平台应支持代码文本提交、文件上传或两者结合的提交方式 | MUST | D/T |
| FR-SUB-03 | 浏览器内编辑 | 对编程类任务，平台应提供基础在线编辑能力 | MUST | D/T |
...
| FR-SUB-07 | 草稿保存与恢复 | 在线编辑场景下，平台应支持草稿暂存和恢复 | SHOULD | T |
...
- BR-OPS-01：审计日志至少应包含操作者、角色、时间、操作类型、操作对象、结果和来源信息。
- BR-OPS-02：审计日志不得被普通业务角色修改或删除。
- BR-OPS-03：审计日志默认保留 1 年，可按机构要求调整。
- BR-OPS-04：以下操作必须进入审计范围：登录/登出、创建/删除用户、角色变更、任务发布/修改、成绩修改、重新评测、导出数据。
...
| 角色 | 必须提供的界面能力 | 说明 |
| --- | --- | --- |
| 学员端 | 登录、加入课程、查看课程与任务、在线提交、查看评测结果、查看成绩与通知、查看个人学习概览 | 核心主链路必须完整可用 |
...
| 数据实体 | 必备信息 | 关键要求 |
| --- | --- | --- |
| PlatformConfig | 平台名称、品牌配置、模块开关、资源策略、版本状态 | 配置变更可追溯且不破坏历史业务数据 |
| OrgUnit | 名称、父级、类型、层级 | 支持组织树结构和用户归属 |
| User | 登录标识、显示名称、组织、角色、账号状态 | 支持账号生命周期和最小必要个人信息 |
...
| 对象 | 状态 | 合法流转 | 说明 |
| --- | --- | --- | --- |
| 平台配置 | draft / published / archived | draft -> published -> archived | 草稿可调整；归档仅供回溯 |
...
| 编号 | 需求 | 衡量标准 | 优先级 | 验证 |
| --- | --- | --- | --- | --- |
| NFR-USE-01 | 浏览器内完成主链路 | 学员无需安装本地开发环境即可完成“查看任务 -> 提交 -> 查看结果”主链路 | MUST | D/T |
| NFR-USE-02 | 明确的状态与错误反馈 | 关键操作必须给出成功、失败或处理中提示，错误信息应可理解 | MUST | D/T |
| NFR-USE-03 | 首次使用可完成主链路 | 首次使用者应能在无专门培训情况下完成“加入课程 -> 查看任务 -> 提交 -> 查看结果” | SHOULD | D/T |
...
| 提交与在线作业 | FR-SUB-01 ~ FR-SUB-07 | UC-05 | Submission | D/T |
