...
1. 启动 Web、API、Worker、数据库、缓存、对象存储和判题服务。
2. 完成初始化数据导入与管理员账号创建。
3. 验证“登录 -> 建课 -> 发任务 -> 提交 -> 评测 -> 批改 -> 出成绩”主链路。
...
| `JUDGE0_URL` | Judge0 API 地址 | `http://judge0:2358` |
...
```bash
docker compose up -d postgres redis minio judge0
```
...
| 提交已创建但一直不评测 | 检查 Redis 队列、Worker 日志和 Judge0 状态 |
| 文件上传失败 | 检查 MinIO 凭据、桶权限和对象大小限制 |
