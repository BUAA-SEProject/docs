...
| NFR-004 | 所有角色 | 控制台/网络 | 关键页面无未解释 console error 和 4xx/5xx | 诊断 | MCP 控制台/网络检查 | 阻塞 | MCP 多个页面存在 console error 条目；新增页面网络摘要见 `product-full-verification-evidence/mcp-network-089-099-teacher-student-content-actions.md`，未逐项完成网络错误归因 | PENDING-MCP |
| NFR-005 | 普通用户 | 表单/列表/工作台 | 不暴露 `orgUnitId`、`scopeRefId`、`studentUserId`、裸 ID、`judgeConfig JSON` 等内部字段 | 可用性 | MCP 页面观察 | 失败 | `product-full-verification-screenshots/093-teacher-discussions-list-before-create.png`; `product-full-verification-screenshots/101-student-short-answer-submission-45-detail.png` | P2-005/P2-006 |
