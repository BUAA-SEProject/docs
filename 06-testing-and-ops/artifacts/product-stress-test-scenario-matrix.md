...
| --- | --- | --- | --- | --- | --- | --- |
| 7.1 | 公共入口、认证与基础读请求 | 读请求 P95 < 500ms、P99 < 1500ms、错误率 < 1%、5xx=0；登录风暴单独统计 | 第九批短诊断跑读请求 50/200 并发 2/3 分钟；修复前 `STRESS-06120151` 跑 50/200/500 | 50 并发 65192 请求全 200、P95 31.21ms、P99 48.70ms；200 并发 133697 请求全 200、P95 439.77ms、P99 2418.45ms、5xx=0。修复前 500 阶段总请求 142801、5xx=1，唯一 500 属于 `teacher_gradebook`。第十批已补成绩册页缓存和 runner stage 超时，但修复后 500/1000 与登录风暴未重测；P99 仍高于 1500ms 的完整阈值。 | 失败 | `product-stress-test-evidence/raw/STRESS-06120105-read-after-hikaritimeout/read_after_hikaritimeout_results.json`；`product-stress-test-evidence/raw/STRESS-06120151-read500-after-stage-timeout/read_ladder_500_after_stage_timeout_results.json`；`product-stress-test-repair-plan.md` |
...
| 7.7 | 真实 go-judge 判题全链路专项 | 真实提交 5/10/20，轮询 50/200/500，重评 5/10，覆盖 5 类结果 | 第九批短诊断跑判题轮询 50/200 并发；数据准备触发 1 次真实编程提交 | 50 并发 75911 请求全 200、P99 14.88ms；200 并发 299536 请求全 200、P99 85.46ms。500 并发、五类结果、报告下载、重评未完成。 | 失败 | `raw/STRESS-06120111-judge-notif-after-hikaritimeout/judge_notif_after_hikaritimeout_results.json`；`manifest.sanitized.json` |
...
| 7.10 | 通知、SSE 与实时恢复专项 | 轮询 50/200/500，SSE 20/100/300 连接保持 5/10/10 分钟 | 第九批短诊断跑通知轮询 50/200；SSE 20 并发 1 分钟沿用首次合同子集 | 50 并发 70158 请求全 200、P99 18.63ms；200 并发 453737 请求全 200、P99 23.28ms。通知 500、SSE 100/300 和 10 分钟保持未完成。 | 失败 | `raw/STRESS-06120111-judge-notif-after-hikaritimeout/judge_notif_after_hikaritimeout_results.json`；`perf_results_contract.json`；`playwright-mcp-post-stress-baseline.md` |
