...
| --- | --- | --- | --- | --- | --- | --- |
| 7.1 | 公共入口、认证与基础读请求 | 读请求 P95 < 500ms、P99 < 1500ms、错误率 < 1%、5xx=0；登录风暴单独统计 | 读请求 50/200/500 并发 2/3/3 分钟；第八批修复后补 read-label 端点级复测 | 首次合同压测 200/500 失败。第八批 no-tx 修复后 50 并发通过；200 并发 P95 251.74ms、错误率 0.01%、5xx 8、P99 2347.58ms；500 并发 P95 2421.80ms、错误率 8.38%、5xx 14238。`auth_me` 200/500 并发 5xx=0，通知未读数 500 并发 5xx=13，`authSessionActiveStack=0`、`authenticatedPrincipalLoaderStack=0`、`notificationServiceStack=26`，登录风暴未单独完成。 | 失败 | `product-stress-test-evidence/raw/STRESS-0611132804/contract-run/perf_results_contract.json`；`product-stress-test-evidence/raw/STRESS-06112149-notifnotx/read-label-results.json`；`product-stress-test-evidence/raw/STRESS-06112149-notifnotx/server-log-window-summary.log`；`server-log-stress-errors-excerpt.log` |
