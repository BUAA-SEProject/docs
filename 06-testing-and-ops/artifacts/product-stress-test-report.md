...
- 运行时模式：基础合同复测使用本地 fake 实验运行时；2026-06-12 第十二批以 `AUBB_LAB_RUNTIME_MODE=kubernetes` 手工启动后端完成真实 Kubernetes Web 终端单会话 smoke，第十三批继续完成真实 Kubernetes WebSocket 5/10/20 并发 10 分钟专项，第十四批补齐真实 go-judge sample-run、提交、报告下载和重评专项，第十五批补齐 runner 对文件/SSE/30 分钟 soak 的合同执行能力，第十六批控制文件上传写入量并补上传完整性 JSON；这些新增场景尚未执行
...
- 未证明或失败范围：读请求 500/1000 长尾仍超过 P95 < 500ms、P99 < 1500ms 的严格阈值；登录风暴未执行；新的 SSE 20/100/300 长连接保持未执行；新的文件上传 20、文件下载 300 和上传/下载 SHA256 矩阵未执行；完整 30 分钟 soak 未执行；Kubernetes CPU/内存曲线因 Metrics API 不可用未记录；多个管理/课程/题库/成绩/权限/前端专项仍未执行压力阶梯。
...
- 交付限制或建议：继续治理读请求长尾，并执行第十五/十六批补齐后的 SSE、文件矩阵和 30 分钟 soak。最新 read ladder 已确认高并发 5xx 清零，但 500/1000 P95/P99 仍超严格阈值；若本地机器无法满足该阈值，需要用新的完整合同结果明确容量边界，而不是只引用旧 smoke 数据。
