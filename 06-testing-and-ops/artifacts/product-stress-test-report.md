...
- 运行时模式：基础合同复测使用本地 fake 实验运行时；2026-06-12 第十二批以 `AUBB_LAB_RUNTIME_MODE=kubernetes` 手工启动后端完成真实 Kubernetes Web 终端单会话 smoke，第十三批继续完成真实 Kubernetes WebSocket 5/10/20 并发 10 分钟专项，第十四批补齐真实 go-judge sample-run、提交、报告下载和重评专项，第十五批补齐 runner 对文件/SSE/30 分钟 soak 的合同执行能力但尚未执行这些新增场景
...
- 结论：不通过全平台合同容量目标。第十一批已修复核心 runner 中的高并发 5xx 和写路径 429；第十二批已打通真实 Kubernetes Web 终端单会话链路并修复学生页启动后不自动刷新 session 的问题；第十三批已证明真实 Kubernetes WebSocket 5/10/20 并发 10 分钟、命令 I/O、重连、重置和停止清理通过；第十四批已证明真实 go-judge sample-run、真实提交、五类结果、报告下载和重评通过；第十五批已把合同 runner 对齐到文件上传 20、文件下载 300、下载 SHA256/size/Content-Type、SSE 20/100/300 长连接和 30 分钟 soak，并让数据准备生成提交附件与实验报告附件 ID。但 500/1000 读请求长尾仍超过 `goal-stress.md` 严格阈值，新 SSE、文件/报表、30 分钟 soak 和多个专项覆盖仍未执行，不能改为通过。
...
- 2026-06-12 第十五批修复结论：`run_perf_suite.py` 合同 profile 已补齐文件上传 5/10/20、文件下载 20/100/300、SSE 20/100/300 长连接保持和 100 并发 30 分钟 soak；文件上传默认循环 64KiB、1MiB、20MiB 三档；文件下载改为专用场景，覆盖课程资源、判题报告、成绩册导出/报告、班级成绩册导出/报告、实验报告附件和提交附件，下载成功后写出 size、Content-Type、SHA256 汇总。`run_sse_scenario` 改为每个 worker 建立一次连接并保持到阶段结束。`setup_perf_data.py` 生成 `submissionArtifactId`、`submissionArtifactSubmissionId`、`labReportAttachmentId`，确保文件矩阵有提交附件和实验报告附件目标。`/opt/miniconda3/bin/python3 -m unittest ops/perf/setup_perf_data_test.py ops/perf/run_perf_suite_test.py` 27 个测试通过。该批未执行压力测试，不能关闭 P2-STRESS-005、7.10 或 7.15。
- 已证明容量边界：读请求 50/200/500/1000 在修复后均 5xx=0；写路径 10/30/50/100 在修复后均 429=0、5xx=0；判题轮询 50/200/500 通过；真实 go-judge sample-run 5/10/20、提交 5/10/20、报告下载、五类结果和重评 5/10 通过；通知轮询 50/200/500 通过；文件上传 10 并发、文件下载 100 并发通过；SSE 20 并发 1 分钟通过；fake runtime 实验会话 10 并发 1 分钟通过；真实 Kubernetes Web 终端 5/10/20 并发 10 分钟通过；100 并发 10 分钟 smoke soak 通过。
- 未证明或失败范围：读请求 500/1000 长尾仍超过 P95 < 500ms、P99 < 1500ms 的严格阈值；登录风暴未执行；新的 SSE 20/100/300 长连接保持未执行；新的文件上传 20、文件下载 300 和 SHA256 矩阵未执行；完整 30 分钟 soak 未执行；Kubernetes CPU/内存曲线因 Metrics API 不可用未记录；多个管理/课程/题库/成绩/权限/前端专项仍未执行压力阶梯。
...
- 交付限制或建议：继续治理读请求长尾，并执行第十五批补齐后的 SSE、文件矩阵和 30 分钟 soak。最新 read ladder 已确认高并发 5xx 清零，但 500/1000 P95/P99 仍超严格阈值；若本地机器无法满足该阈值，需要用新的完整合同结果明确容量边界，而不是只引用旧 smoke 数据。
...
| 文件上传下载压测 | 旧执行：上传 5/10、下载 20/100；新 runner：上传 5/10/20、下载 20/100/300 | 旧执行上传各 1 分钟、下载各 2 分钟；新 runner 未执行 | 旧执行上传 16.86/31.93；下载 221.27/729.38 | 旧执行上传 17.53/21.53 ms；下载 19.16/68.41 ms | 旧执行上传 24.43/30.34 ms；下载 24.00/106.99 ms | 0 | 0 | 阻塞 | `raw/STRESS-0612022949/contract-run/perf_results_contract.json`；第十五批 runner 已补 20MiB 上传分层和下载 SHA256/size/Content-Type 汇总，数据准备已补提交附件和实验报告附件 ID，但新文件矩阵未执行。 |
...
- 清理建议：以 `STRESS-0611131857`、`STRESS-0611132135`、`STRESS-0611132804`、`STRESS-0612022949`、`STRESS-0612045100-k8s-runtime`、`STRESS-0612053038-k8s-ws`、`STRESS-0612064213-gojudge` 前缀清理本地压测组织、账号、课程、作业、资源和实验数据；清理前保留当前 artifacts 作为证据。

## 11. 最终建议

最终建议：不通过全平台合同容量。第十一批已经把核心 runner 中的高并发 5xx 和写路径 429 修复，并用 targeted read/write 与 10 分钟 smoke soak 证明；第十二批打通真实 Kubernetes Web 终端单会话，并修复学生页启动后不轮询导致页面卡在“正在启动”的缺陷；第十三批进一步证明 Kubernetes WebSocket 5/10/20 并发 10 分钟通过；第十四批补齐真实 go-judge sample-run、提交、五类结果、报告下载和重评。但 500/1000 读请求长尾仍超过严格阈值，SSE 100/300、文件/报表专项、30 分钟完整 soak、Kubernetes CPU/内存曲线和 7.2-7.15 多个专项仍未关闭。下一轮应优先治理读请求长尾，并补 SSE、文件/报表和剩余域专项容量。
